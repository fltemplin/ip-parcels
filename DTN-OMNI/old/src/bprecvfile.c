...
static int	receiveFile(Sdr sdr, BpDelivery *dlv, int fileNum)
{
	...
	fileCount++;
	isprintf(fileName, sizeof fileName, "testfile%d", fileNum);
...
int	main(int argc, char **argv)
{
	...
	int		fileNum = (argc > 3 ? strtol(argv[3], NULL, 0) : 0);
#endif
	...
	if (ownEid == NULL)
	{
		PUTS("Usage: bprecvfile <own endpoint ID> [fileNum]");
		return 0;
	}
	...
	while (state.running)
	{
		...
		case BpPayloadPresent:
			filesReceived++;
			fileNum++;
			if (receiveFile(sdr, &dlv, fileNum) < 0)
			{
