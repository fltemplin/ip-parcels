...
#ifdef OMNI_MTU
	/* Unconditionally set IPv6 tunnel endpoint destination path MTU to
	 * IPV6_MAXPLEN (i.e., regardless of actual dst_mtu) so that IPv6
	 * fragmentation of tunnel packet will be invoked if necessary. */

	mtu = IPV6_MAXPLEN - eth_hlen - psh_hlen - t->tun_hlen;
#else
...
#ifdef OMNI_MTU
	/* Do not update original source PMTU; IPv6 fragmentation of
	 * tunnel packet will be invoked if necessary.  */
#else
...
#ifdef OMNI_MTU
	/* Expand ip6tunnel_xmit() and leading portion of __ip6_local_out()
	 * inline (see: <net/ip6_tunnel.h>), and substitute ip6_output() for
	 * rest of ip6_local_out() */

	{
		int pkt_len, err2;

		/* GSO applies to payload pkt (not tunnel pkt). Resetting GSO
		 * and setting ignore_df allows tunnel ptk fragmentation */
		skb_gso_reset (skb);
		skb->ignore_df = 1;

		memset(skb->cb, 0, sizeof(struct inet6_skb_parm));
		pkt_len = skb->len - sizeof(struct ipv6hdr);
		if (pkt_len > IPV6_MAXPLEN)
			pkt_len = 0;
		ipv6_hdr(skb)->payload_len = htons(pkt_len);
		IP6CB(skb)->nhoff = offsetof(struct ipv6hdr, nexthdr);
		pkt_len = skb->len - skb_inner_network_offset(skb);

		err2 = ip6_output(dev_net(skb_dst(skb)->dev), sk, skb);

		if (unlikely(net_xmit_eval(err2)))
			pkt_len = -1;
