...
int __ip_local_out(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	/* Parcels have true length in Jumbo Payload option */
	if (skb_shinfo(skb)->parcel_segs)
		iph->tot_len = htons(skb_shinfo(skb)->parcel_slen);
	else
			 ...
			 struct ipcm_cookie *ipc, struct rtable **rtp)
{
	...
	cork->parcel_slen = ipc->parcel_slen;
	cork->parcel_segs = ipc->parcel_segs;
			      ...
			      struct inet_cork *cork)
{
	...
	if (cork->parcel_segs) {
		skb_shinfo(skb)->parcel_segs = cork->parcel_segs;
		skb_shinfo(skb)->parcel_slen = cork->parcel_slen;
	}
